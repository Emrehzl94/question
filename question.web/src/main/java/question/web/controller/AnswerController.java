...
public class AnswerController {
	...
	@RequestMapping(value="/answers/{uid}/{qid}", method=RequestMethod.POST)
	public Answer createAnswer(@PathVariable int uid,@PathVariable int qid,@RequestBody Answer answer){
		Answer a = new Answer();
		a = answer;
		a.setUser(userRepo.findOne(uid));
		a.setQuestion(questionRepo.findOne(qid));
		a.setApproved(false);
		a.setDate(new Date());
		answerRepo.save(a);
		return a;
